				...
				success:function(replyResult){
					...
					$(replyResult).each(function(i, coment){
						...
						if(coment.userid=='${logId}'){//'dong' == 'asd'
							...
							tag += "<input type='button' value='삭제' title='"+coment.board_r_no+"'/>";
							tag += "<div id='replyContent'>"+coment.coment+"</div></div>"; //댓글내용
							//수정 폼
...
<main>
	<input type="hidden" value="${dto.board_no}">
	<div id="commuView">
		<div>
			<div class="comViewMa">
			...
			<div id="commuInfo">
				<div style="font-size:1.5em; margin-bottom: 10px;">[${dto.board_cate}] ${dto.board_title}</div>
		...
		<div style="width:100%;">
			<div id="btn">
				...
				<c:if test="${logId == dto.userid}">
					<p><button onclick="location.href='<%=request.getContextPath() %>/Board/BoardEdit?board_no=${dto.board_no}'">게시글 수정하기</button></p>
